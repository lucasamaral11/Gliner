...
def organizar_links_e_precos(dados_json, texto_bruto):
    ...
    lojas_permitidas = ["amazon", "shopee", "aliexpress", "mercadolivre", "meli", "magaziniluiza", "magalu", "casasbahia", "girafa", "kabum", "pichau", "terabyte", "link.amazon"]
    ...
    # REGRA SOLICITADA: Se o link do produto for igual ao link do cupom, anula o link do cupom
    ...
    for campo in ["preco_atual", "preco_anterior"]:
        ...
        if valor is None or str(valor).strip().lower() in ["null", "none", ""]:
            ...
            valor_str = str(valor)
            # CORREÇÃO DA LINHA DE MÉTODOS MISTURADOS (.split corrigido para .replace)
            valor_str = valor_str.replace('.00', '').replace(',00', '')
